...
def test_peso_cross_tipo_pesa_mais_que_mono_tipo():
    """Mesmo volume e nº de buckets: cross-tipo > mono-tipo."""
    cross = calcular_peso(volume_total=10, n_buckets=2, n_tipos=2)  # 10*2*(1+1.0)=40
    mono = calcular_peso(volume_total=10, n_buckets=2, n_tipos=1)  # 10*2*(1+0.5)=30
    assert cross == 40.0
    assert mono == 30.0
    assert cross > mono
...
def test_detecta_cruzamento_literal_cross_tipo(client_loyall, db_session):
    ...
    assert c["peso"] == calcular_peso(3, 2, 2)  # 3*2*(1+1.0)=12
